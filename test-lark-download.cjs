...
async function test() {
  ...
  // Test with a dummy file_token or fetch one from Bitable first
  // Let's fetch one from Bitable
  ...
  console.log("Records data code:", recordsData.code);
  ...
  if (!fileRes.ok) {
    ...
    console.log("Success! Headers:", fileRes.headers.raw());
  }
}
