...
## V2 报告与表单升级（2026-07-10 下午，已上线）

- **报告页重设计**（Trust & Authority 模式，手机优先）：六大类雷达图 SVG + 保障完整度/缺失板块/月保费三大数字 + 每类一句话朋友语气点评 + 顾问介绍区（持牌财务规划师定位 + 「不强推产品」信任徽章）+ 限时优惠 CTA（RM88，原价 RM888，7 天有效，6 项 what-you-get，DuitNow QR base64 内嵌 + TNG 收款资料 + Remark 填自己名字提醒）；tagline「买得对，买得够，每一分保费花在刀口上」；移除 WhatsApp 按钮（拿到链接者已有联系方式）
- **第六类改为「财富传承」**：仅当保单属信托保单（trust policy）才评级，否则一律「待确认」；级别枚举「未知」改为「待确认」
- **AI 语气改版**：note ≤40 字口语化、先肯定后指出；建议只给方向（≤25 字）；总评 ≤90 字轻松收尾引向咨询
- **表单双通道**：上传保单改为可选；新增手动填写保障数额（人寿/重疾/医疗年限额/意外/年缴总额，可向现有 agent 索取）；无文件时走 manual-only 管道（跳过提取，AI 按自报数额评估并标注）
- **稳定性加固**：Gemini 全部切 flash-lite-latest（flash-latest 免费层高峰长期 503）+ 45s 超时 + 5 次重试；Supabase 节点 5 次重试；n8n 容器钉死公共 DNS（Docker Desktop DNS 间歇故障）；错误报警工作流须处于激活状态（n8n 2.x 要求）
