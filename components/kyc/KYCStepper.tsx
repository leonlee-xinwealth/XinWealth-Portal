...
export const STEPS = [
    ...
    { id: 'pdpa', label: 'PDPA Consent', showNav: false },
...
const KYCStepper: React.FC = () => {
    ...
    const renderStepContent = () => {
        switch (currentStep.id) {
            ...
            case 'pdpa':
                return <PDPAStep pdpaAccepted={formData.pdpaAccepted} updateData={updateFormData} onNext={handleNext} />;
            case 'basic':
    ...
    return (
        <div className={`flex flex-col lg:flex-row gap-8 items-start ${currentStep.id === 'pdpa' ? 'justify-center' : ''}`}>
            
            {/* Main Form Content */}
            <div className={`w-full ${currentStep.showNav ? 'lg:w-[70%]' : (currentStep.id === 'welcome' || currentStep.id === 'pdpa' ? 'max-w-3xl' : 'lg:w-full')} transition-all duration-300`}>
                {renderStepContent()}
            </div>
