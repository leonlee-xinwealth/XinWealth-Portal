                  ...
                  <div key={r.key} className="bg-slate-50 rounded-xl p-3 mb-2 relative">
                    ...
                    {r.category === 'medical' && (
                      <div className="mt-1 pt-2 border-t border-slate-200">
                        {tiers.length > 0 && (
                          <Fr label={t('Tier','等级')}>
                            <Sel value={r.tier_name} onChange={(v: string) => { const tier = tiers.find((tt: any) => tt.tier_name===v); if (tier) applyTier(idx, tier); else setRider(idx,{tier_name:v}); }} opts={[['','—'],...tiers.map((tt: any): [string, string] => [tt.tier_name, tt.tier_name])]} />
                          </Fr>
                        )}
